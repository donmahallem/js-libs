...
export const createSudokuBoard = (boardSize: number, retainPercentage: number): number[][] => {
    ...
    const randomNumbers: number[] = new Array<number>(boardSize)
        .fill(0)
        .map((val: number, idx: number): number => idx + 1)
    ...
    const removeIndexes: number[] = getIndexMap(boardSize).sort((): number => Math.random() - 0.5);
