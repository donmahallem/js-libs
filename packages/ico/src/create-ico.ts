...
export const generateIco = <T extends Type = Type.ICON>(data: IIcoDataInput<T>): Buffer => {
    ...
    const dataSize: number = (data.images as any)
        .reduce((prev: number, cur: InputEntry<T>): number => {
    ...
    for (let i: number = 0; i < data.images.length; i++) {
        ...
        } as EntryType<T>, entry);
        writeDirEntry(fileBuffer,
            FILE_HEADER_SIZE + (i * DIRECTORY_SIZE),
            entryData,
            data.type);
        /**
         * Copy File Data
