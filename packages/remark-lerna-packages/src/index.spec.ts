...
describe('index', (): void => {
    describe('base config', (): void => {
        it('should set all plugins with default config', async (): Promise<void> => {
            ...
            const processor: Processor = unified().use(remarkParse).use(remarkGfm).use(remarkLernaPlugin, { lernaConfig: './test' });
            const parsedData: Node = processor.parse(data);
            return processor.run(parsedData).then((value: Node): void => {
    ...
    describe('createRowFromPackage', (): void => {
        it('should create row with homepage and title', (): void => {
            ...
            const row: Node = createRowFromPackage(
                Package.lazy({
                        ...
                        children: [
                            {
                                type: 'text',
                                value:
                                    '<a href="https://badge.fury.io/js/pck1"><img alt="npm version"' +
